...
template <class T>
[[nodiscard]] std::optional<T> parseBase58Wrapper(std::string const &str) {
...
template <class T>
[[nodiscard]] std::optional<T> parseBase58Wrapper(xrpl::TokenType type,
...
accountFromStringStrict(std::string const &account) {
  ...
  if (blob && xrpl::publicKeyType(xrpl::makeSlice(*blob))) {
    ...
    publicKey = parseBase58Wrapper<xrpl::PublicKey>(
...
ledgerTypeInfoFromStr(std::string const &entryName) {
  ...
  static auto const kRPC_MAP = []() {
    ...
    for (auto const &e : kLEDGER_TYPES_TABLE)
      m.emplace(std::string{e.rpcName},
                LedgerTypeInfo{.type = e.type, .category = e.category});
    return m;
  }();
