        ...
        if (xrpl::isXRP(currency)) {
            ...
            if (hasIssuer)
                return std::unexpected{rpc::Status{
                    IsrErr,
                    "Unneeded field '" + std::string{f.key()} + ".issuer' for XRP currency specification."
                }};
...
struct TakerConverter {
    ...
    parse(FA const& f) const
    {
        auto const err = [] {
            return std::unexpected{rpc::Status{
                rpc::RippledError::RpcInvalidParams, "Invalid field 'taker'."
            }};
        };
        if (!f.isString())
            return err();
...
inline constexpr auto kInputSpec = spec<Input>(
    ...
    field("taker", &Input::taker, withCustomError(account, RippledError::RpcInvalidParams, "Invalid field 'taker'."), takerConv),
    field(
        "taker_pays",
        &Input::takerPays,
        required,
