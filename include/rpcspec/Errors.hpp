 ...
 *
 * Higher value = better progress made before failure; LoadBalancer picks std::max.
 * Defined here so Status and CombinedError can include them without a Clio ETL dep.
 * Clio's etl::EtlError aliases this type.
 */
enum class EtlError {
...
/**
 * @brief Clio operates on a combination of Rippled, custom Clio, and ETL error codes.
 *
...
struct Status {
    CombinedError code = ripple::rpcSUCCESS;
    std::string error;
    std::string message;
