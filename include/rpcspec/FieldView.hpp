...
public:
  ...
  [[nodiscard]] bool present() const noexcept { return readValue_ != nullptr; }
  ...
  [[nodiscard]] uint32_t asUint32() const {
    return readValue_->is_uint64()
               ? static_cast<uint32_t>(readValue_->as_uint64())
  ...
  [[nodiscard]] bool asBool() const { return readValue_->as_bool(); }
  ...
  [[nodiscard]] double asDouble() const { return readValue_->as_double(); }
...
public:
  ...
  [[nodiscard]] BoostJsonFieldView child(std::string_view key) const noexcept {
    ...
    return BoostJsonFieldView{static_cast<boost::json::value const *>(nullptr),
                              key};
  }
};
