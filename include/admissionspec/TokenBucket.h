...
#include <admissionspec/folly/TokenBucket.h>
...
private:
    ...
    // folly derives the token count from (now - zeroTime) * rate, capped at burst. To start full at
    // `nowSeconds`, place zeroTime far enough in the past that the bucket has already filled to
    // capacity.
    [[nodiscard]] static double
