...
public:
    ...
    {
        auto _ = std::scoped_lock<std::mutex>{mutex_};
        auto& buckets = state_.buckets;
        for (auto it = std::begin(buckets); it != std::end(buckets);)
        {
            if (now - it->second.lastSeen >= idleFor)
            {
                it = buckets.erase(it);
            }
            else
...
private:
    ...
    // Find @p conn's bucket, creating it (evicting the oldest if at capacity) if absent, and stamp
    // it as seen at @p now. Caller must hold @p buckets' lock.
    ...
    charge(ConnId const& conn, AdmissionDecision const& decision, TimePoint now)
    {
        ...
        if (!it->second.bucket.tryConsume(decision.tokenCost, now))
        {
    ...
    // Apply a dropped stage's penalty cost to @p conn's bucket and return the drop unchanged. The
    // debit is best-effort: if the bucket lacks the tokens it is left as-is (the connection is
    // already at its limit). A drop is always a drop regardless of bucket state.
    [[nodiscard]] AdmissionDecision
