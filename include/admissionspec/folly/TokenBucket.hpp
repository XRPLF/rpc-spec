...
inline constexpr std::size_t kTokenBucketHardwareDestructiveInterferenceSize =
    128;

struct TokenBucketPolicyDefault {
  using align = std::integral_constant<
      size_t,
      kTokenBucketHardwareDestructiveInterferenceSize>;
...
class BasicDynamicTokenBucket {
 ...
 public:
  ...
  bool consume(
    ...
    double const consumed = bucket_.consume(
   ...
   * Thread-safe.
   */
  std::optional<double> consumeWithBorrowNonBlocking(
      double toConsume,
      double rate,
      double burstSize,
      double nowInSeconds = defaultClockNow()) {
...
class BasicTokenBucket {
 ...
 public:
   ...
   * Thread-safe (but returned value may immediately be outdated).
   */
  [[nodiscard]] double available(double nowInSeconds = defaultClockNow()) const noexcept {
   ...
   * Thread-safe (but returned value may immediately be outdated).
   */
  [[nodiscard]] double balance(double nowInSeconds = defaultClockNow()) const noexcept {
   ...
   * Thread-safe (but returned value may immediately be outdated).
   */
  [[nodiscard]] double burst() const noexcept { return burstSize_; }
