...
// NOTICE: This file has been modified from the original folly source
// (folly/TokenBucket.h). It is vendored here and trimmed to remove the
// dependency on the rest of the folly library so the admission rate limiter
// can use it without taking a dependency on Folly::folly. See LICENSE and
// README.md in this directory. The token-bucket logic is unchanged; only the
// few folly utilities it relied on have been inlined:
//   - folly::ConstexprMath (constexpr_min/constexpr_max) -> std::min/std::max
...
#define XRPL_ADMISSION_FOLLY_UNLIKELY(x) (__builtin_expect((x), 0))
#else
#define XRPL_ADMISSION_FOLLY_UNLIKELY(x) (x)
#endif

namespace folly {

// folly exposes this via folly/concurrency/CacheLocality.h; inlined here as a
// plain constant to avoid pulling in that header. 128 matches folly's default
// and is a safe cache-line padding to avoid false sharing of zeroTime_.
inline constexpr std::size_t kTokenBucketHardwareDestructiveInterferenceSize = 128;

struct TokenBucketPolicyDefault
{
    using align = std::integral_constant<size_t, kTokenBucketHardwareDestructiveInterferenceSize>;
...
public:
     ...
     */
    explicit TokenBucketStorage(double zeroTime = 0) noexcept : zeroTime_(zeroTime)
    {
    }
     ...
     * however.)
     */
    TokenBucketStorage(TokenBucketStorage const& other) noexcept
     ...
     */
    TokenBucketStorage&
    operator=(TokenBucketStorage const& other) noexcept
    {
     ...
     */
    [[nodiscard]] double
    balance(double rate, double burstSize, double nowInSeconds) const noexcept
    {
        assert(rate > 0);
        assert(burstSize > 0);
        double const zt = this->zeroTime_.load(std::memory_order_relaxed);
    ...
    double
    consume(double rate, double burstSize, double nowInSeconds, Callback const& callback)
    {
        assert(rate > 0);
        assert(burstSize > 0);
        ...
        do
        {
            ...
            double const tokensNew = tokens - consumed;
            if (consumed == 0.0)
            {
                return consumed;
            }
        ...
        } while (XRPL_ADMISSION_FOLLY_UNLIKELY(
    ...
    double
    timeWhenBucket(double rate, double target)
    {
     ...
     */
    void
    returnTokens(double tokensToReturn, double rate)
    {
        assert(rate > 0);
...
private:
     ...
     */
    double
    returnTokensImpl(double tokenCount, double rate)
    {
        ...
        double zeroTime)
    {
        if (Concurrent::value)
        {
            return atom.compare_exchange_weak(expected, zeroTime, std::memory_order_relaxed);
        }
    ...
    static constexpr size_t AlignZeroTime = std::max<size_t>(Align::value, alignof(Atom<double>));
...
public:
     ...
     *                 buckets are "empty" after construction.
     */
    explicit BasicDynamicTokenBucket(double zeroTime = 0) noexcept : bucket_(zeroTime)
    {
    }
     ...
     * however.)
     */
    BasicDynamicTokenBucket(BasicDynamicTokenBucket const& other) noexcept = default;
    BasicDynamicTokenBucket&
    operator=(BasicDynamicTokenBucket const& other) noexcept = default;
        ...
        double nowInSeconds = defaultClockNow())
    {
        ...
        double const consumed =
            bucket_.consume(rate, burstSize, nowInSeconds, [toConsume](double available) {
        ...
        double nowInSeconds = defaultClockNow())
    {
        ...
        double consumed =
            bucket_.consume(rate, burstSize, nowInSeconds, [toConsume](double available) {
                return std::min(available, toConsume);
            });
        return consumed;
    }
     ...
     *
     * Returns a std::optional<double>. The optional wont be set if the request
        ...
        double nowInSeconds = defaultClockNow())
    {
        ...
        if (res.value_or(0) > 0)
        {
            auto const napUSec = static_cast<int64_t>(res.value() * 1000000);
     ...
     */
    [[nodiscard]] double
    balance(double rate, double burstSize, double nowInSeconds = defaultClockNow()) const noexcept
    {
...
public:
     ...
     */
    BasicTokenBucket(BasicTokenBucket const& other) noexcept = default;
     ...
     */
    BasicTokenBucket&
    operator=(BasicTokenBucket const& other) noexcept = default;

    /**
     * Returns the current time in seconds since Epoch.
     */
    static double
    defaultClockNow() noexcept(noexcept(Impl::defaultClockNow()))
    {
     ...
     */
    void
    reset(double genRate, double burstSize, double nowInSeconds = defaultClockNow()) noexcept
    {
        assert(genRate > 0);
        assert(burstSize > 0);
        double const availTokens = available(nowInSeconds);
        rate_ = genRate;
     ...
     */
    void
    setCapacity(double tokens, double nowInSeconds) noexcept
    {
     ...
     */
    bool
    consume(double toConsume, double nowInSeconds = defaultClockNow())
    {
     ...
     */
    double
    consumeOrDrain(double toConsume, double nowInSeconds = defaultClockNow())
    {
     ...
     */
    void
    returnTokens(double tokensToReturn)
    {
     ...
     */
    std::optional<double>
    consumeWithBorrowNonBlocking(double toConsume, double nowInSeconds = defaultClockNow())
    {
     ...
     */
    bool
    consumeWithBorrowAndWait(double toConsume, double nowInSeconds = defaultClockNow())
    {
     ...
     */
    [[nodiscard]] double
    available(double nowInSeconds = defaultClockNow()) const noexcept
    {
     ...
     */
    [[nodiscard]] double
    balance(double nowInSeconds = defaultClockNow()) const noexcept
    {
...
#undef XRPL_ADMISSION_FOLLY_UNLIKELY
