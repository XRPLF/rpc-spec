...
/// Convert a tunable default value to its resolved runtime representation.
template <typename T>
...
tunableIndex() noexcept
{
    ...
    std::size_t i = 0;
    ((Tunables::kNAME == Name.view() ? (void)(idx = i) : (void)0, ++i), ...);
    return idx;
}
...
/// Maps a `std::tuple<Tunable...>` to the corresponding @ref ResolvedTunables type.
...
/// Build a @ref ResolvedTunables from a tunable tuple using each tunable's default value.
template <typename... Tunables>
